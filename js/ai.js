/* ============================================================
   x402-ai-agent.space — AI assistant widget
...
async function handleSend(){
  ...
  // Пытаемся через шлюз OpenRouter (бесплатная модель).
  // Шлюз ИИ — на том же домене (worker-site.js отдаёт /api/chat).
  // Никакого cross-origin, никакой зависимости от второго субдомена.
  ...
  try{
    ...
    try {
      ...
      removeTyping();
      // Показываем реальную сетевую ошибку для диагностики
      addMsg('⚠️ Сетевая ошибка шлюза: ' + String(netErr).slice(0,150) + '\n(локальный ответ) ' + localAnswer(text), 'bot');
      return;
    }
    removeTyping();
    if (res.ok){
      const j = await res.json();
      if (j.reply) { addMsg(j.reply, 'bot'); return; }
    }
    // Показываем ответ сервера при ошибке (для диагностики)
    let detail = '';
    try { const ej = await res.json(); detail = JSON.stringify(ej).slice(0,200); } catch(e){}
    addMsg('⚠️ Шлюз вернул ' + res.status + ': ' + detail + '\n(локальный ответ) ' + localAnswer(text), 'bot');
  }catch(e){
    removeTyping();
